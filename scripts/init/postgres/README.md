...
- `scripts/init/postgres/run_all.sh`
  - 初期化スクリプトを順番にすべて実行します。

- `scripts/init/postgres/scripts/001_create_database.sh`
  - デフォルト DB `postgres` に接続し、`PGDATABASE` で指定した DB がなければ作成します。

- `scripts/init/postgres/scripts/002_create_schema.sh`
  - `PGDATABASE` に接続し、`core` スキーマを作成します（未作成時のみ）。

- `scripts/init/postgres/scripts/003_roles.sh`
  - API 用ロール（`core` スキーマ管理）を作成し、最小権限を付与し、`PUBLIC` 権限を制限します。

- `scripts/init/postgres/scripts/004_search_path.sh`
...
```bash
bash scripts/init/postgres/run_all.sh
```
...
```bash
bash scripts/init/postgres/scripts/001_create_database.sh
```

##### 2-2) スキーマ作成

```bash
bash scripts/init/postgres/scripts/002_create_schema.sh
```

##### 2-3) ロール作成・権限設定

```bash
bash scripts/init/postgres/scripts/003_roles.sh
```

##### 2-4) `search_path` 既定値設定

```bash
bash scripts/init/postgres/scripts/004_search_path.sh
```
